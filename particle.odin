...
// SoA 레이아웃: 접근 패턴별로 버퍼 분리 → GPU 캐시 효율 최대화
// pos/vel: compute R/W + render read
// color/size: 초기화 후 정적 (render read only)
Particle_System :: struct {
	pos:    ^sdl3.GPUBuffer, // [MAX_PARTICLES][2]f32
	vel:    ^sdl3.GPUBuffer, // [MAX_PARTICLES][2]f32
	color:  ^sdl3.GPUBuffer, // [MAX_PARTICLES][4]f32
	size:   ^sdl3.GPUBuffer, // [MAX_PARTICLES]f32
	active: int,
}

// Compute shader uniform (set=1, binding=0)
Compute_UBO :: struct {
	bounds_x: f32,
	bounds_y: f32,
	count:    u32,
