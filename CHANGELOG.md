...
### 파티클 형태 설계 메모 (Phase 2+ 참고)

효과 목적에 따라 파티클 렌더 형태가 달라져야 한다는 분석:

| 효과 타입 | 추천 형태 | 이유 |
|-----------|-----------|------|
| 디졸브/분해 | 큐브 (Cube) | 물질감을 위해 정육면체가 유리. 원거리에서는 쿼드로 LOD 전환하여 성능 절감 |
| 댄싱 파티클 | 포인트/쿼드 | 개별 형태보다 전체 흐름이 중요. 가장 가벼운 선택 (현재 구조 그대로) |
| 헤어/스트링 | 리본 (Ribbon Strip) | 실린더는 폴리곤 과다. 얇은 직사각형 Strip 연결로 털 질감 표현 |
...
|------|-----------|-----------|
| 포인트/쿼드 | 이미 구현됨 | — |
| 큐브 (기본) | 가능 | 3D 위치(vec3), MVP 행렬, 깊이 버퍼 |
| 큐브 + LOD | 불가 | 카메라 시스템 (거리 계산 기반 전환) |
| 리본/스트링 | 불가 | 체인 데이터 구조 (파티클 간 순서 정보), 현재 SoA는 독립 개체 가정 |

**핵심 제약:**
- 큐브 인스턴싱 시 버텍스 수 = 36 × 파티클 수. 1M 파티클이면 3600만 버텍스/프레임
- LOD 전환은 카메라 거리 계산이 필요 → 카메라 시스템이 선행되어야 함
- 리본은 파티클을 "N개 묶음의 순서 있는 체인"으로 보는 새 데이터 모델 필요
