...
`particle_basic_single`에 Vsync + 전체화면 전환을 적용한 예제.
...
`SetWindowFullscreen`은 bool 하나로 전환됩니다.
- `true` → 현재 모니터 해상도로 전체화면 (borderless fullscreen)
- `false` → 원래 창 크기로 복귀

Vsync는 전체화면 전환 후에도 그대로 유지됩니다.
swapchain present mode는 윈도우 상태와 무관하게 device에 유지되기 때문입니다.
